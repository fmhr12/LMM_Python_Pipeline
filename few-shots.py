...
# 3. REFERENCE IMAGES PATH (Few-Shot)
REF_DIR = Path('path')
...
- These images have **bounding boxes visually drawn on the carious teeth**.
- **Calibrate:** Adjust your sensitivity to match the visual appearance of caries in these reference images.
- **Mimic:** Ensure your output bounding boxes match the tightness and style of these references.
...
- **Format:** Output a list of 5 numbers: `[Confidence_Score, ymin, xmin, ymax, xmax]`
  - `Confidence_Score`: Float between 0.0 and 1.0.
  - `ymin, xmin, ymax, xmax`: Integers between 0 and 1000.

### SCANNING EXECUTION
Scan the image from **Left to Right**, looking at one tooth at a time.
**Do not wait until the end.** Report your findings immediately as you scan.

### INSTRUCTIONS
1.  **Start at the far left tooth.**
2.  **Isolate:** Focus ONLY on a single tooth at a time.
3.  **Analyze:** With high sensitivity to any sign of caries, does the tooth have caries based on ICDAS criteria and reference standards?
    - **IF YES (Potential Caries):**
        ...
        - **IF CONFIRMED:**
            ...
            - Format: `>>> DETECTED: [confidence_score, ymin, xmin, ymax, xmax]`
        - **IF REJECTED:** Do nothing.
4.  **Move to the next tooth.**
5.  **Repeat** until you reach the far right tooth.

### CRITICAL RULES
- **Immediate Reporting:** Output the `>>> DETECTED:` line the exact moment you find caries.
- **Strict Coordinates:** The box must cover ONLY the single tooth you are currently scanning.
- **Interproximal Caries:** If caries are between Tooth A and B, output separate boxes for each tooth. NEVER draw one box for more than one tooth.

### EXAMPLE OUTPUT STREAM
[Start Scan]
>>> DETECTED: [0.95, 150, 200, 190, 240]
>>> DETECTED: [0.60, 155, 300, 195, 340]
[End Scan]
"""
...
def load_reference_images(ref_dir):
    """Loads all images from the reference directory to use as few-shot examples."""
    ref_parts = []
    if not ref_dir.exists():
        print(f"⚠️ Warning: Reference directory not found at {ref_dir}")
        return []
    
    valid_exts = ['.jpg', '.png', '.jpeg']
    files = [f for f in ref_dir.iterdir() if f.suffix.lower() in valid_exts and not f.name.startswith('.')]
    
    print(f"   -> Loading {len(files)} reference images for few-shot learning...")
    for f in files:
        ref_parts.append(get_file_content(f))
    return ref_parts

def call_gemini(prompt, image_path, ref_parts):
    # Construct content: Prompt -> Reference Examples -> Target Image
    contents = [prompt]
    
    if ref_parts:
        # We add a text label so the model knows these are the references mentioned in the prompt
        contents.append("\n### START OF REFERENCE IMAGES (GOLD STANDARD)\n")
        contents.extend(ref_parts)
        contents.append("\n### END OF REFERENCES. START SCANNING TARGET IMAGE BELOW:\n")
...
def calculate_map(all_preds, total_gt_count):
    ...
    This smooths the Precision-Recall curve by taking the max precision 
    to the right of the current recall level.
    """
    ...
    # 3. Calculate Cumulative Sums (Accumulate counts down the list)
    # Example: [1, 0, 1] becomes [1, 1, 2]
    ...
    # We prepend 0.0 to start the graph at origin, and append sentinel at end
    precisions = [0.0] + precisions + [0.0]
    recalls    = [0.0] + recalls + [1.0]

    # 6. SMOOTHING (The "Interpolation" Step)
    # We go BACKWARDS through the list.
    # At each point, we ask: "What is the highest precision achieved AFTER this point?"
    # This flattens the valleys in the graph (Max-to-the-right).
    for i in range(len(precisions) - 2, -1, -1):
        precisions[i] = max(precisions[i], precisions[i + 1])

    # 7. Calculate Area Under Curve (AUC)
    # We sum the rectangular areas under the smoothed curve
    ap = 0.0
    for i in range(1, len(recalls)):
        # Width of the step (change in recall)
        recall_change = recalls[i] - recalls[i-1]
        
        # Only calculate area if there is a step forward in recall
        if recall_change > 0:
            # Area = Width * Height (Height is the smoothed precision)
...
print("Loading Reference Images (Few-Shot)...")
ref_parts = load_reference_images(REF_DIR)
...
print(f"--- STARTING OCCLUSAL ANALYSIS ---")
...
for i, raw_file in enumerate(files_to_process):
    ...
    # PREDICT
    pred_text = call_gemini(PROMPT_OCCLUSAL, raw_file, ref_parts)
